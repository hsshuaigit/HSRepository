...
@property(nonatomic,strong)NSMutableArray *danmuArr;
...
-(void)getDanmuData{
    [_renderer start];
    [NetTool getDataWithSuburl:self.url Parameters:self.param Success:^(NSURLSessionDataTask *dataTask, id responseObject) {
        ...
        if ([dic[@"status"] integerValue]==1) {
            ...
            if (arr.count>0) {
                self.danmuArr=nil;
                self.danmuArr=dic[@"result"];
                for (NSDictionary *dic in self.danmuArr) {
                    NSLog(@"弹幕为===%@",dic[@"message"]);
                    BarrageDescriptor * descriptor = [[BarrageDescriptor alloc]init];
                    descriptor.spriteName = NSStringFromClass([BarrageWalkTextSprite class]);
                    descriptor.params[@"text"] = [NSString stringWithFormat:@"%@",dic[@"message"]];
                    descriptor.params[@"textColor"] = kRandomColor;
...
- (void)setupSubViews {
    ...
    //_danmuTimer=[NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(getDanmuData) userInfo:nil repeats:YES];
    //测试
    [_renderer start];
    _timer = [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(autoSendBarrage) userInfo:nil repeats:YES];

    [self.playerLayerView addSubview:_renderer.view];
